package com.xuegao.multi_thread2.lock;
...
/**
 * <br/> @PackageName：com.xuegao.multi_thread2.lock
 * <br/> @ClassName：StampedLockTest
 * <br/> @Description：
 * <br/> @author：xuegao
 * <br/> @date：2020/9/26 0:02
 */
public class StampedLockTest {
    // 对于读多写少的高并发场景 StampedLock的性能很好，通过乐观读模式很好的解决了写线程“饥饿”的问题，
    // 我们可以使用StampedLock 来代替ReentrantReadWriteLock ，但是需要注意的是 StampedLock 的功能仅仅是 ReadWriteLock 的子集，
    // 在使用的时候，还是有几个地方需要注意一下。
    //
    // StampedLock是不可重入锁，使用过程中一定要注意；
    // 悲观读、写锁都不支持条件变量 Conditon ，当需要这个特性的时候需要注意；
    // 如果线程阻塞在 StampedLock 的 readLock() 或者 writeLock() 上时，此时调用该阻塞线程的 interrupt() 方法，
    // 会导致 CPU 飙升。所以，使用 StampedLock 一定不要调用中断操作，如果需要支持中断功能，
